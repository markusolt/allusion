...
//! `std::sync::Weak` is reference counted. For this reason weak references can be duplicated
//! with no performance cost. To access the referenced value you must first upgrade the weak
//! references though, which similary to [`std::sync::Weak::upgrade`] performs multiple atomic
//! operations. It may be cheaper to simply clone [`std::sync::Arc`]s around to avoid the cost
//! of upgrading a weak reference.
//!
